...
const UserProfile = () => {
    ...
    const handleRemoveFriendClick = async () => {
        ...
        if (response.success) {
            const friendship = auth.user.friendships.filter(friend => friend.to_user._id === userId)
        ...
        <div className='w-5/6 rounded-md md:w-1/3 border-2 p-5  my-8 mx-auto flex flex-col bg-slate-100 md:flex-row' onSubmit={() => { }}>
            <div className='md:px-8 w-full flex flex-col items-center justify-center'>
                ...
                <div className='w-full mt-2 flex flex-col items-end'>
                    {
                        checkFriend ?
                            (<button className='bg-cyan-500 w-full mb-2 text-white p-2 rounded-md' onClick={handleRemoveFriendClick}>
                                {requestInProgress ? 'Removing friend. . .' : 'Remove friend'}
                            </button>)
                            : (
                                <button className='bg-cyan-500 my-4 w-full mt-2 text-white p-2 rounded-md' onClick={handleAddFriendClick}>
